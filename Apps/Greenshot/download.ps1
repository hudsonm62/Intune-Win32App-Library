...
param(
    [Parameter()]
    [uri]$URI = (& (Join-Path $PSScriptRoot '../../scripts/Get-LatestGitHubRelease.ps1') -Repo 'greenshot/greenshot' -Filter {$_.name -like '*RELEASE.exe'}).DownloadURI,
    [Parameter()]
    [string]$OutFileName = 'Greenshot-Installer.exe'
)
