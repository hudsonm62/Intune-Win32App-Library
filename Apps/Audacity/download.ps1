...
param(
    [Parameter()]
    [uri]$URI = (& (Join-Path $PSScriptRoot '../../scripts/Get-LatestGitHubRelease.ps1') -Repo 'audacity/audacity' -Filter {$_.name -like '*64bit.exe'}).DownloadURI,
    [Parameter()]
    [string]$OutFileName = 'Audacity-Installer.exe'
)
