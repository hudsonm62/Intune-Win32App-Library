...
param(
    [Parameter()]
    [uri]$URI = (& (Join-Path $PSScriptRoot '../../scripts/Get-LatestGitHubRelease.ps1') -Repo 'obsidianmd/obsidian-releases' -Filter {$_.name -like '*.exe'}).DownloadURI,
    [Parameter()]
    [string]$OutFileName = 'Obsidian-Installer.exe'
)
