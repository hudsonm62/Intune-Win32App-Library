...
WARNING:
    ...
    - If you want to use the ARM or 32bit version of an installer, you'll need to update the link yourself.
...
param(
    [Parameter()]
    [uri]$URI = (& (Join-Path $PSScriptRoot '../../scripts/Get-LatestGitHubRelease.ps1') -Repo 'ip7z/7zip' -Filter {$_.name -like '*x64.exe'}).DownloadURI,
    [Parameter()]
    [string]$OutFileName = "7z-installer.exe"
)
