#region Setup
$AppName = "Microsoft .NET Runtime"
$Locations = @(
    # User Locations
    [string](Join-path $env:LOCALAPPDATA "dotnet\dotnet.exe")

    # System Locations
    [string](Join-path $env:ProgramFiles "dotnet\dotnet.exe")
    [string](Join-path ${env:ProgramFiles(x86)} "dotnet\dotnet.exe")
)
#endregion
